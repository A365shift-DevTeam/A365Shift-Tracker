...
const SortableContactCard = ({ contact, onEdit, onDelete, onPreview }) => {
  ...
  const style = {
    ...
    position: 'relative',
    touchAction: 'none',
    // Disable hover transforms when being dragged by dnd-kit
...
export const KanbanView = ({
  ...
  const resolveOverColumn = useCallback((overId) => {
    ...
    // Is it a contact? Find its column
  ...
  const handleDragEnd = (event) => {
    ...
    // FIX: Actually assign newStatus when dropping on a column
    if (columns.includes(overId)) {
      newStatus = overId
    } else {
      // Dropped on another contact — use that contact's column
    ...
    >
      <Row className="kanban-container g-3 flex-nowrap overflow-auto py-2">
        {columns.map((column, index) => {
          // Highlight the target column while dragging
